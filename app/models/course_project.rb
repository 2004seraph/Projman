  ...
  def self.lifecycle_job
    ...
    CourseProject.where.not(status: %i[draft completed archived]).find_each do |c|
      ...
      if (c.team_size == 1) || c.team_allocation.nil?
        ...
        if c.teammate_preference_deadline && (c.teammate_preference_deadline.deadline < DateTime.now && !c.teammate_preference_deadline.executed)
          ...
          Rails.logger.debug group_matrix
